...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-rose-100 shadow-2xs w-full overflow-x-hidden">
      {/* Main Navigation Bar */}
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20 gap-2 sm:gap-4">
          ...
          {/* Desktop Navigation Links - Shown only on XL (1280px+) so it never pushes cart off-screen */}
          <nav className="hidden xl:flex items-center space-x-6 2xl:space-x-8 text-xs font-semibold uppercase tracking-wider text-stone-600 shrink-0">
            <a
              href="#collection"
          ...
          {/* Right Action Icons & Direct Order - Guaranteed Always Visible & Perfectly Sized */}
          <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
            {/* Bridal Party Suite Button (Desktop & Tablet) */}
            <button
              type="button"
              onClick={onOpenBridalSuite}
              className="hidden lg:inline-flex items-center gap-2 px-3.5 py-2 rounded-full bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-200 text-xs font-bold tracking-wider uppercase transition-all shadow-2xs hover:shadow-xs whitespace-nowrap shrink-0 cursor-pointer"
            >
            ...
            {/* Shopping Cart Button - Always visible, never pushed off */}
            <button
              type="button"
              onClick={onOpenCart}
            ...
            {/* Navigation Toggle for Mobile & Tablet (<1280px) */}
            <button
              type="button"
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="p-2 rounded-md xl:hidden text-stone-700 hover:text-rose-600 hover:bg-rose-50 transition-colors shrink-0"
      ...
      {/* Drawer Menu (< 1280px) */}
      {isMobileMenuOpen && (
        <div className="xl:hidden bg-white border-b border-rose-100 px-4 pt-3 pb-6 space-y-4 shadow-lg animate-in slide-in-from-top-2">
